...
export async function listProjects(): Promise<Project[]> {
  const { data, error } = await supabase
    .from("projects")
    .select(
      "*, project_members!inner(user_id, role), tasks(id, status, task_assignees(user_id))",
    )
    .is("archived_at", null)
...
export async function createProject(
  ...
  const data = await invokeAuthenticatedFunction<{ project: Project }>(
    "create-project",
    {
...
export async function addProjectMember(
  ...
  const data = await invokeAuthenticatedFunction<{ member: ProjectMember }>(
    "sync-project-member",
    {
      body: {
        projectId,
...
export async function rewrapProjectMemberKey(
  projectId: string,
  target: Pick<Profile, "id" | "encryption_public_key">,
): Promise<void> {
  if (!target.encryption_public_key)
...
export async function removeProjectMember(
  projectId: string,
  userId: string,
): Promise<{ githubSyncStatus: string; githubErrorCode: string | null }> {
...
export async function updateProject(
  projectId: string,
  updates: Partial<
    Pick<Project, "name" | "description" | "github_auto_sync" | "status">
  >,
): Promise<Project> {
  const { data, error } = await supabase
    .from("projects")
  ...
  try {
    ...
    const messages: Record<string, string> = {
      ...
      STORAGE_CLEANUP_FAILED:
        "프로젝트 파일을 정리하지 못했습니다. 다시 시도해 주세요.",
      PROJECT_DELETE_FAILED:
...
export async function retryGitHubMemberSync(
  projectId: string,
  userId: string,
...
export async function retryGitHubRepositoryCreation(
  projectId: string,
): Promise<Project> {
  const data = await invokeAuthenticatedFunction<{ project: Project }>(
    "github-retry",
