...
export function LoginPage() {
  ...
  return <main className="min-h-screen bg-canvas px-4 py-8 sm:py-14"><div className="mx-auto mb-10 max-w-6xl"><Brand /></div><div className="mx-auto grid max-w-6xl overflow-hidden rounded-3xl border border-line bg-surface shadow-soft lg:grid-cols-[1.1fr_.9fr]">
    <section className="hidden min-h-[650px] flex-col justify-between bg-slate-950 p-10 text-white lg:flex"><div><div className="eyebrow text-blue-300">Private by design</div><h1 className="mt-5 max-w-lg text-4xl font-extrabold leading-tight tracking-tight">우리 프로젝트를<br />한곳에서, 안전하게.</h1><p className="mt-5 max-w-md text-sm leading-7 text-slate-300">프로젝트 권한은 Supabase RLS가 보호하고 민감한 설명·댓글·파일은 브라우저에서 프로젝트별 AES-256-GCM 키로 암호화합니다.</p></div><div className="grid gap-3"><div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-4"><ShieldCheck className="text-blue-300" /><div><div className="text-sm font-bold">관리자 발급 계정</div><div className="text-xs text-slate-400">공개 회원가입 없이 학번으로 로그인</div></div></div><div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-4"><LockKeyhole className="text-emerald-300" /><div><div className="text-sm font-bold">Project-level access</div><div className="text-xs text-slate-400">멤버가 아닌 프로젝트는 API에서도 차단</div></div></div></div></section>
    <section className="flex min-h-[580px] items-center p-6 sm:p-10 lg:p-12"><form onSubmit={submit} className="w-full"><div className="mb-8"><div className="eyebrow">Welcome back</div><h2 className="mt-2 text-3xl font-extrabold tracking-tight text-ink">로그인</h2><p className="mt-2 text-sm text-muted">관리자가 발급한 학번과 비밀번호를 입력하세요.</p></div>{!isSupabaseConfigured && <Alert tone="info" className="mb-4">`.env`에 Supabase URL과 Publishable Key를 설정해야 로그인할 수 있습니다.</Alert>}{error && <Alert className="mb-4">{error}</Alert>}<label className="label" htmlFor="student-id">학번</label><Input id="student-id" value={studentId} onChange={(event) => setStudentId(event.target.value)} inputMode="numeric" autoComplete="username" placeholder="20260001" pattern="[0-9]{6,12}" required className="mb-4" /><label className="label" htmlFor="password">비밀번호 / PIN</label><Input id="password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" placeholder="••••" minLength={4} required /><Button type="submit" size="lg" className="mt-6 w-full" disabled={loading || !isSupabaseConfigured}>{loading ? <><Spinner className="h-4 w-4 border-white/30 border-t-white" /> 인증 중</> : <><KeyRound size={17} /> 로그인</>}</Button><p className="mt-5 text-center text-xs leading-5 text-muted">이메일 주소는 필요하지 않습니다. 신규 계정의 초기 비밀번호는 1234입니다.</p></form></section>
  </div></main>;
}
