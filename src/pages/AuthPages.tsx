...
function Brand() {
  return <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl border border-white/20 bg-brand text-white shadow-lg shadow-brand/20"><FolderKanban size={19} /></div><div><div className="font-extrabold tracking-tight text-ink">Team Rocket</div><div className="text-[10px] font-bold tracking-[.12em] text-muted">프로젝트 관리</div></div></div>;
}
...
export function LoginPage() {
  ...
  return <main className="min-h-screen px-4 py-8 sm:py-14"><div className="mx-auto mb-8 max-w-6xl"><Brand /></div><div className="auth-card mx-auto grid max-w-6xl overflow-hidden rounded-[28px] lg:grid-cols-[1.1fr_.9fr]">
    <section className="auth-ambient hidden min-h-[640px] flex-col justify-between border-r border-line/70 p-10 text-ink lg:flex"><div><div className="eyebrow">프로젝트 관리</div><h1 className="mt-5 max-w-lg text-4xl font-extrabold leading-tight tracking-tight">우리 프로젝트를<br />한곳에서, 더 명확하게.</h1><p className="mt-5 max-w-md text-sm leading-7 text-muted">프로젝트와 작업, 일정, 담당자, 진행 상황을 한곳에서 관리하고 팀의 흐름을 빠르게 확인하세요.</p><div className="mt-8 flex gap-3 text-brand"><FolderKanban /><CheckSquare /><Users /></div></div><div className="grid gap-3"><div className="subtle-panel flex items-center gap-3 p-4"><FolderKanban className="text-brand" /><div><div className="text-sm font-bold">업무 흐름 한눈에</div><div className="text-xs text-muted">칸반·일정·진행률을 프로젝트 단위로 정리</div></div></div><div className="subtle-panel flex items-center gap-3 p-4"><CheckSquare className="text-brand" /><div><div className="text-sm font-bold">팀 협업 정리</div><div className="text-xs text-muted">담당자·댓글·체크리스트·활동 기록을 한곳에서</div></div></div></div></section>
...
export function FirstLoginPage() {
  ...
  if (!initialized) return <main className="flex min-h-screen items-center justify-center"><Spinner /></main>;
  ...
  return <main className="flex min-h-screen items-center justify-center p-4"><section className="auth-card w-full max-w-xl rounded-3xl p-6 sm:p-9"><div className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl border border-brand/15 bg-brand/10 text-brand shadow-sm"><KeyRound /></div><div className="eyebrow">첫 로그인</div><h1 className="mt-2 text-2xl font-extrabold text-ink">처음 로그인하셨습니다.</h1><p className="mt-2 text-sm leading-6 text-muted">{profile.name}님, 프로젝트를 사용하기 전에 새 PIN 또는 비밀번호를 설정해 주세요.</p><p className="mt-2 text-xs leading-5 text-muted">이전 변경이 중단된 뒤 다시 들어왔다면, 로그인할 때 사용한 새 PIN 또는 비밀번호를 그대로 입력하면 안전하게 완료됩니다.</p>
    <form onSubmit={submit} className="mt-7"><fieldset><legend className="label">로그인 방식</legend><div className="grid gap-3 sm:grid-cols-2"><label className={`cursor-pointer rounded-xl border p-4 ${mode === "pin" ? "border-brand bg-brand/5" : "border-line bg-raised/30"}`}><input type="radio" name="password-mode" value="pin" checked={mode === "pin"} onChange={() => { setMode("pin"); setPassword(""); setConfirmation(""); }} className="mr-2" /><span className="text-sm font-bold text-ink">4자리 PIN</span><p className="ml-6 mt-1 text-xs text-muted">숫자 정확히 4자리</p></label><label className={`cursor-pointer rounded-xl border p-4 ${mode === "password" ? "border-brand bg-brand/5" : "border-line bg-raised/30"}`}><input type="radio" name="password-mode" value="password" checked={mode === "password"} onChange={() => { setMode("password"); setPassword(""); setConfirmation(""); }} className="mr-2" /><span className="text-sm font-bold text-ink">일반 비밀번호</span><p className="ml-6 mt-1 text-xs text-muted">문자·숫자·특수문자, 4자 이상</p></label></div></fieldset>
...
export function UnlockPage() {
  ...
  const navigate = useNavigate();
  const [password, setPassword] = useState("");
  if (!initialized || !keyringHydrated) return <main className="flex min-h-screen items-center justify-center"><Spinner /></main>;
