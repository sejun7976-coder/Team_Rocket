  ...
  return (
    <div className="page-wrap">
      ...
      {projects.isLoading ? <div className="flex min-h-72 items-center justify-center"><Spinner /></div> : projects.error ? <div className="panel p-5 text-sm text-red-600">{projects.error.message}</div> : <>
        ...
        {active.length === 0 ? <div className="mt-3"><EmptyState icon={<FolderKanban />} title="아직 프로젝트가 없습니다" description={admin ? "첫 팀 프로젝트를 만들고 업무를 정리하세요." : "관리자가 프로젝트에 초대하면 여기에 표시됩니다."} action={admin ? <Button onClick={() => setCreateOpen(true)}><Plus size={16} /> 새 프로젝트</Button> : undefined} /></div> : <div className="mt-3 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {active.map((project) => {
