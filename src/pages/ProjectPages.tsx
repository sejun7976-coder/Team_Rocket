...
import { Activity, CalendarDays, CheckSquare, CircleDot, FileText, Github, LayoutDashboard, ListTodo, MessageSquare, MoreHorizontal, Plus, Search, Settings, ShieldAlert, Users } from "lucide-react";
...
function TaskFormDialog({ open, onClose, projectId, members }: { open: boolean; onClose: () => void; projectId: string; members: ProjectMember[] }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["tasks", projectId] }); onClose(); setTitle(""); setDescription(""); setAssigneeIds([]); }
  });
  const submit = (event: FormEvent) => { event.preventDefault(); mutation.mutate({ projectId, title, description, priority, dueDate: dueDate || undefined, assigneeIds }); };
  return <Modal open={open} onClose={onClose} title="새 작업" description="민감한 설명은 브라우저에서 암호화됩니다."><form onSubmit={submit} className="space-y-4">{mutation.error && <div className="rounded-xl bg-red-500/10 p-3 text-sm text-red-600">{mutation.error.message}</div>}<div><label className="label" htmlFor="task-title">제목</label><Input id="task-title" value={title} onChange={(event) => setTitle(event.target.value)} maxLength={240} required autoFocus /></div><div><label className="label" htmlFor="task-description">설명</label><textarea id="task-description" className="field min-h-24" value={description} onChange={(event) => setDescription(event.target.value)} /></div><div className="grid grid-cols-2 gap-3"><div><label className="label" htmlFor="task-priority">우선순위</label><select id="task-priority" className="field" value={priority} onChange={(event) => setPriority(event.target.value as TaskPriority)}><option value="low">낮음</option><option value="medium">보통</option><option value="high">높음</option><option value="urgent">긴급</option></select></div><div><label className="label" htmlFor="task-due">마감일</label><Input id="task-due" type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} /></div></div><fieldset><legend className="label">담당자 (복수 선택)</legend><div className="max-h-40 space-y-1 overflow-y-auto rounded-xl border border-line p-2">{members.map((member) => <label key={member.user_id} className="flex cursor-pointer items-center gap-3 rounded-lg p-2 hover:bg-raised"><input type="checkbox" checked={assigneeIds.includes(member.user_id)} onChange={(event) => setAssigneeIds((current) => event.target.checked ? [...current, member.user_id] : current.filter((id) => id !== member.user_id))} /><Avatar name={member.profile?.name ?? "팀원"} url={member.profile?.avatar_url} size="sm" /><span className="text-sm font-medium text-ink">{member.profile?.name}</span></label>)}</div></fieldset><div className="flex justify-end gap-2"><Button variant="secondary" onClick={onClose}>취소</Button><Button type="submit" disabled={mutation.isPending}>{mutation.isPending ? <Spinner /> : "작업 생성"}</Button></div></form></Modal>;
}
