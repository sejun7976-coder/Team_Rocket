...
import { FolderKanban, History, KeyRound, MoreVertical, Plus, Settings2, ShieldCheck, Sparkles, Trash2, Users } from "lucide-react";
...
import { addAIModel, deleteAIKey, deleteAIModel, getAISettings, saveAIProvider, testAIConnection, updateAIModel, type AIProvider } from "../services/ai";
...
function DeleteUserDialog({ target, onClose }: { target: AdminUser | null; onClose: () => void }) {
  ...
  const mutation = useMutation({ mutationFn: () => deleteAdminUser(target!.id, confirmation.trim()), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["admin-users"] }); onClose(); } });
  return <Modal open={Boolean(target)} onClose={onClose} title="사용자 완전 삭제" description="이 작업은 Auth 계정과 멤버십을 제거하며 되돌릴 수 없습니다.">
    <Alert>프로젝트 소유자와 system_admin은 삭제할 수 없습니다. GitHub collaborator 정리가 실패하면 삭제도 중단됩니다.</Alert>
    <label className="label mt-4" htmlFor="delete-user-confirmation">확인을 위해 학번 {target?.student_id} 입력</label><Input id="delete-user-confirmation" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="off" />
    {mutation.error && <Alert className="mt-4">{mutation.error.message}</Alert>}
    <div className="mt-5 flex justify-end gap-2"><Button variant="secondary" onClick={onClose}>취소</Button><Button variant="danger" disabled={confirmation.trim() !== target?.student_id || mutation.isPending} onClick={() => mutation.mutate()}><Trash2 size={14} /> 완전 삭제</Button></div>
  </Modal>;
}
...
export function AdminUsersPage() {
  ...
  return <div className="page-wrap">
    ...
    {users.isLoading ? <Spinner /> : users.data?.length ? <div className="space-y-2">{users.data.map((managedUser) => <article key={managedUser.id} className="panel grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 p-3 md:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)_minmax(0,.9fr)_auto]"><div className="min-w-0"><div className="truncate font-bold text-ink">{managedUser.name}</div><div className="truncate font-mono text-[11px] text-muted">{managedUser.student_id}{managedUser.github_username ? ` · @${managedUser.github_username}` : ""}</div></div><div className="hidden min-w-0 flex-wrap gap-1 md:flex"><Badge tone={statusTones[managedUser.status]}>{statusLabels[managedUser.status]}</Badge><Badge tone={managedUser.system_role === "admin" ? "purple" : "neutral"}>{managedUser.system_role}</Badge></div><div className="hidden min-w-0 md:block"><p className="truncate text-[11px] text-muted">최근 {displayDate(managedUser.lastSignInAt)}</p><p className="truncate text-[10px] text-muted">{managedUser.recentIpAddress ?? "IP 없음"} · 30일 {managedUser.loginCount30Days}회</p></div><details className="relative justify-self-end"><summary aria-label={`${managedUser.name} 관리 메뉴`} className="flex h-9 w-9 cursor-pointer list-none items-center justify-center rounded-lg hover:bg-raised"><MoreVertical size={17} /></summary><div className="absolute right-0 z-20 mt-1 w-48 rounded-xl border border-line bg-surface p-1 shadow-lift"><button className="w-full rounded-lg px-3 py-2 text-left text-xs hover:bg-raised" onClick={() => setAccessTarget(managedUser)}>접속 기록 보기</button><button className="w-full rounded-lg px-3 py-2 text-left text-xs hover:bg-raised disabled:opacity-40" disabled={managedUser.id === currentUser?.id || managedUser.status === "inactive"} onClick={() => setResetTarget(managedUser)}>비밀번호 초기화</button><button className="w-full rounded-lg px-3 py-2 text-left text-xs hover:bg-raised disabled:opacity-40" disabled={managedUser.id === currentUser?.id || statusMutation.isPending} onClick={() => toggleStatus(managedUser)}>{managedUser.status === "inactive" ? "재활성화" : "비활성화"}</button><button className="w-full rounded-lg px-3 py-2 text-left text-xs text-red-600 hover:bg-red-500/10 disabled:opacity-40" disabled={managedUser.id === currentUser?.id || managedUser.system_role === "admin"} onClick={() => setDeleteTarget(managedUser)}>완전 삭제</button></div></details><div className="col-span-2 flex gap-1 md:hidden"><Badge tone={statusTones[managedUser.status]}>{statusLabels[managedUser.status]}</Badge><Badge tone={managedUser.system_role === "admin" ? "purple" : "neutral"}>{managedUser.system_role}</Badge></div></article>)}</div> : <EmptyState icon={<Users />} title="등록된 사용자가 없습니다" description="첫 사용자를 추가하세요." />}
...
export function AdminSystemPage() {
  return <div className="page-wrap">
    ...
    <div className="grid gap-4 md:grid-cols-2"><section className="panel p-5"><div className="flex items-center gap-2 text-ink"><ShieldCheck className="text-brand" size={20} /><h2 className="font-extrabold">Auth 정책</h2></div><dl className="mt-4 space-y-3 text-sm"><div className="flex justify-between"><dt className="text-muted">Public signup</dt><dd className="font-bold text-ink">비활성화</dd></div><div className="flex justify-between"><dt className="text-muted">Minimum password length</dt><dd className="font-bold text-ink">6</dd></div><div className="flex justify-between"><dt className="text-muted">초기 비밀번호</dt><dd className="font-mono font-black text-ink">1234</dd></div><div className="flex justify-between"><dt className="text-muted">내부 이메일 노출</dt><dd className="font-bold text-ink">없음</dd></div></dl></section><section className="panel p-5"><div className="flex items-center gap-2 text-ink"><Settings2 className="text-brand" size={20} /><h2 className="font-extrabold">운영 확인</h2></div><ul className="mt-4 space-y-3 text-sm text-muted"><li>• Migration 001~009 순서대로 적용</li><li>• Authentication Audit Logs DB 저장 활성화</li><li>• FRONTEND_URL과 service role secret 확인</li><li>• 최초 관리자 one-time bootstrap 완료</li></ul></section></div>
    <Alert tone="info" className="mt-5">실제 Supabase Dashboard 설정 절차는 SUPABASE_SETUP.md에 정리되어 있습니다.</Alert>
  </div>;
}

const providerLabels: Record<AIProvider, string> = { openai: "OpenAI", anthropic: "Anthropic", google: "Google AI" };

export function AdminAISettingsPage() {
  const queryClient = useQueryClient();
  const settings = useQuery({ queryKey: ["admin-ai-settings"], queryFn: getAISettings });
  const [keys, setKeys] = useState<Record<AIProvider, string>>({ openai: "", anthropic: "", google: "" });
  const [newModel, setNewModel] = useState({ provider: "openai" as AIProvider, modelId: "", displayName: "" });
  const refresh = async () => { setKeys({ openai: "", anthropic: "", google: "" }); await queryClient.invalidateQueries({ queryKey: ["admin-ai-settings"] }); await queryClient.invalidateQueries({ queryKey: ["ai-models"] }); };
  const providerMutation = useMutation({ mutationFn: (input: { provider: AIProvider; enabled: boolean }) => saveAIProvider({ ...input, ...(keys[input.provider].trim() ? { apiKey: keys[input.provider].trim() } : {}) }), onSuccess: refresh });
  const removeKey = useMutation({ mutationFn: deleteAIKey, onSuccess: refresh });
  const test = useMutation({ mutationFn: testAIConnection });
  const addModel = useMutation({ mutationFn: () => addAIModel({ ...newModel, modelId: newModel.modelId.trim(), displayName: newModel.displayName.trim(), enabled: true }), onSuccess: async () => { setNewModel((value) => ({ ...value, modelId: "", displayName: "" })); await refresh(); } });
  const editModel = useMutation({ mutationFn: updateAIModel, onSuccess: refresh });
  const removeModel = useMutation({ mutationFn: ({ provider, id }: { provider: AIProvider; id: string }) => deleteAIModel(provider, id), onSuccess: refresh });
  const error = settings.error ?? providerMutation.error ?? removeKey.error ?? test.error ?? addModel.error ?? editModel.error ?? removeModel.error;
  return <div className="page-wrap max-w-5xl"><PageHeader eyebrow="Admin" title="AI Provider & Model" description="API Key는 provider별로 서버 암호화 저장되며, 사용자는 관리자가 등록한 모델만 선택할 수 있습니다." />
    {error && <Alert className="mb-4">{error.message}</Alert>}
    {settings.isLoading ? <Spinner /> : <><div className="grid gap-4 lg:grid-cols-3">{(["openai", "anthropic", "google"] as AIProvider[]).map((provider) => { const item = settings.data?.providers.find((value) => value.provider === provider); return <section key={provider} className="panel p-4"><div className="flex items-center justify-between"><h2 className="font-extrabold text-ink">{providerLabels[provider]}</h2><Badge tone={item?.configured ? "green" : "neutral"}>{item?.configured ? "Key 설정됨" : "미설정"}</Badge></div><label className="label mt-4" htmlFor={`${provider}-key`}>API Key 교체</label><Input id={`${provider}-key`} type="password" autoComplete="new-password" value={keys[provider]} onChange={(event) => setKeys((value) => ({ ...value, [provider]: event.target.value }))} placeholder="저장 후 다시 표시되지 않음" /><div className="mt-4 flex flex-wrap gap-2"><Button size="sm" onClick={() => providerMutation.mutate({ provider, enabled: true })}>저장·활성화</Button><Button size="sm" variant="secondary" disabled={!item?.configured} onClick={() => test.mutate(provider)}>테스트</Button><Button size="sm" variant="ghost" className="text-red-600" disabled={!item?.configured} onClick={() => { if (confirm(`${providerLabels[provider]} Key를 삭제할까요?`)) removeKey.mutate(provider); }}><Trash2 size={13} /></Button>{item?.enabled && <Button size="sm" variant="ghost" onClick={() => providerMutation.mutate({ provider, enabled: false })}>비활성화</Button>}</div></section>; })}</div>
      <section className="panel mt-5 p-5"><div className="flex items-center gap-2"><Sparkles className="text-brand" size={19} /><h2 className="font-extrabold text-ink">허용 모델 레지스트리</h2></div><form className="mt-4 grid gap-2 md:grid-cols-[160px_1fr_1fr_auto]" onSubmit={(event) => { event.preventDefault(); addModel.mutate(); }}><select className="field" value={newModel.provider} onChange={(event) => setNewModel((value) => ({ ...value, provider: event.target.value as AIProvider }))}>{Object.entries(providerLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><Input value={newModel.modelId} onChange={(event) => setNewModel((value) => ({ ...value, modelId: event.target.value }))} placeholder="Provider model ID" required /><Input value={newModel.displayName} onChange={(event) => setNewModel((value) => ({ ...value, displayName: event.target.value }))} placeholder="사용자 표시 이름" required /><Button type="submit" disabled={addModel.isPending}><Plus size={14} /> 추가</Button></form><div className="mt-4 space-y-2">{settings.data?.models.map((model) => <div key={model.id} className="grid gap-2 rounded-xl border border-line p-3 sm:grid-cols-[1fr_auto] sm:items-center"><div className="min-w-0"><p className="truncate text-sm font-bold text-ink">{model.display_name} <span className="font-normal text-muted">· {providerLabels[model.provider]}</span></p><p className="truncate font-mono text-[11px] text-muted">{model.model_id}</p></div><div className="flex flex-wrap gap-1"><Button size="sm" variant={model.enabled ? "secondary" : "ghost"} onClick={() => editModel.mutate({ provider: model.provider, modelSettingId: model.id, enabled: !model.enabled })}>{model.enabled ? "활성" : "비활성"}</Button><Button size="sm" variant={model.is_default ? "secondary" : "ghost"} disabled={model.is_default} onClick={() => editModel.mutate({ provider: model.provider, modelSettingId: model.id, isDefault: true })}>기본 모델</Button><Button size="sm" variant="ghost" className="text-red-600" onClick={() => { if (confirm("모델을 삭제할까요?")) removeModel.mutate({ provider: model.provider, id: model.id }); }}><Trash2 size={13} /></Button></div></div>)}</div></section></>}
  </div>;
}
