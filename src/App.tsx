...
import { BoardPage, ProjectLayoutPage, ProjectOverviewPage, TaskListPage } from "./pages/ProjectPages";
import { ProjectActivityPage, ProjectCalendarPage, ProjectFilesPage, ProjectGitHubPage, ProjectSettingsPage, ProjectTeamPage } from "./pages/ProjectSecondaryPages";
...
function LoadingScreen() {
  return <div className="flex min-h-screen items-center justify-center bg-canvas"><Spinner /></div>;
}
...
function AdminGuard() {
  ...
  return isSystemAdmin(user, profile) ? <Outlet /> : <Navigate to="/dashboard" replace />;
}
...
function Bootstrap({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!user) return;
    const channel = supabase.channel(`account-status:${user.id}`).on(
      "postgres_changes",
      ...
      (payload) => {
        ...
        forgetAll();
        queryClient.clear();
        void logout().finally(() => { window.location.hash = "#/login"; });
      }
    ).subscribe();
...
export function App() {
  return <HashRouter><Bootstrap><Routes>
