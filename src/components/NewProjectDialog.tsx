  ...
  return (
    <Modal open={open} onClose={onClose} title="새 프로젝트" description="팀 업무 공간을 먼저 만들고 GitHub 연동은 선택할 수 있습니다." className="max-w-xl">
      <form onSubmit={submit} className="space-y-4">
        ...
        <div><label className="label" htmlFor="project-name">프로젝트 이름</label><Input id="project-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="AI Pilot" maxLength={120} required autoFocus /></div>
        ...
        <label className="flex items-center justify-between rounded-xl border border-line bg-raised p-3">
          <div><div className="flex items-center gap-2 text-sm font-bold text-ink"><Github size={16} /> GitHub Repository 자동 생성</div><div className="mt-1 text-xs text-muted">선택하지 않아도 프로젝트의 모든 업무 기능을 사용할 수 있습니다.</div></div>
        ...
        {createRepository && <div className="space-y-4 rounded-xl border border-line p-4">
          <div><label className="label" htmlFor="repository-name">Repository 이름</label><div className="relative"><Github className="pointer-events-none absolute left-3 top-3 text-muted" size={17} /><Input id="repository-name" value={repositoryName} onChange={(event) => { setRepositoryEdited(true); setRepositoryName(event.target.value); }} pattern="[A-Za-z0-9._-]{1,100}" className="pl-10 font-mono" placeholder="ai-pilot" required /></div></div>
          <fieldset><legend className="label">공개 범위</legend><div className="grid grid-cols-2 gap-3">
            <button type="button" onClick={() => setVisibility("private")} className={`rounded-xl border p-3 text-left transition ${visibility === "private" ? "border-brand bg-brand/5" : "border-line bg-surface hover:bg-raised"}`}><div className="flex items-center gap-2 text-sm font-bold text-ink"><Lock size={15} /> Private</div><div className="mt-1 text-xs text-muted">기본값 · 팀원만 접근</div></button>
            <button type="button" onClick={() => setVisibility("public")} className={`rounded-xl border p-3 text-left transition ${visibility === "public" ? "border-brand bg-brand/5" : "border-line bg-surface hover:bg-raised"}`}><div className="flex items-center gap-2 text-sm font-bold text-ink"><Users size={15} /> Public</div><div className="mt-1 text-xs text-muted">누구나 코드 열람 가능</div></button>
          </div></fieldset>
        </div>}
        <Alert tone="info"><strong>Owner는 자동으로 나로 설정됩니다.</strong><br />GitHub 생성이 실패해도 프로젝트는 정상 생성되며 설정에서 복구할 수 있습니다.</Alert>
