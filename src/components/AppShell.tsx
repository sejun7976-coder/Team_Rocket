...
import { Activity, Bell, CalendarDays, CheckSquare, FolderCog, FolderKanban, LayoutDashboard, LogOut, Menu, Settings, Shield, SlidersHorizontal, Sparkles, Users, X } from "lucide-react";
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-canvas"><div className="fixed inset-y-0 left-0 z-40 hidden lg:block">{sidebar}</div>{mobileOpen && <div className="fixed inset-0 z-50 lg:hidden"><button aria-label="메뉴 닫기" className="absolute inset-0 bg-slate-950/45" onClick={() => setMobileOpen(false)} /><div className="relative h-full w-[270px]">{sidebar}<Button variant="ghost" className="absolute right-3 top-4 h-8 w-8 p-0" onClick={() => setMobileOpen(false)}><X size={18} /></Button></div></div>}<div className="lg:pl-[250px]"><header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-line bg-surface/90 px-4 backdrop-blur sm:px-6"><div className="flex items-center gap-3"><Button variant="ghost" className="h-9 w-9 p-0 lg:hidden" onClick={() => setMobileOpen(true)}><Menu size={19} /></Button><div className="hidden text-sm font-semibold text-muted sm:block">안녕하세요, <span className="text-ink">{profile?.name}</span>님</div></div><div className="flex items-center gap-1.5"><Button variant="ghost" className="h-9 gap-2 px-2.5" aria-label="Rocket AI 열기" title="Rocket AI (Ctrl/⌘+K)" onClick={openRocketAI}><Sparkles size={17} /><span className="hidden xl:inline">Rocket AI</span></Button><Button variant="ghost" className="relative h-9 w-9 p-0" aria-label="알림" onClick={() => navigate("/notifications")}><Bell size={18} />{unread > 0 && <span className="absolute right-1 top-1 h-2 w-2 rounded-full bg-red-500" />}</Button><ThemeCycleButton className="border-transparent" /><div className="ml-2 flex items-center gap-2 border-l border-line pl-3"><Avatar name={profile?.name ?? "사용자"} url={profile?.avatar_url} size="sm" /><div className="hidden md:block"><div className="max-w-28 truncate text-xs font-bold text-ink">{profile?.name}</div><div className="text-[10px] text-muted">{profile?.student_id}</div></div>{admin ? <Badge tone="purple" className="hidden xl:inline-flex">system admin</Badge> : profile?.github_username && <Badge tone="neutral" className="hidden xl:inline-flex">@{profile.github_username}</Badge>}</div></div></header><main>{children}</main></div><RocketAIPanel /></div>;
}
