...
serve(async (request) => {
  ...
  const { user, admin } = await requirePermission(context, ADMIN_PERMISSIONS.USERS_DELETE);
  ...
  const { data: profile } = await admin.from("profiles").select("id, student_id, name, github_username, system_role, account_status").eq("id", targetId).maybeSingle();
  ...
  if (profile.account_status === "active") {
    const { data: managers, error: managerError } = await admin
      ...
      .select("user_id, profile:profiles!inner(account_status)")
      .eq("permission", ADMIN_PERMISSIONS.USERS_MANAGE_PERMISSIONS)
      .eq("profile.account_status", "active");
    if (managerError) throw new ApiError(500, "USER_DELETE_CHECK_FAILED", "권한 관리자 상태를 확인할 수 없습니다.");
    if (managers?.some((manager) => manager.user_id === targetId) && managers.length === 1) {
      throw new ApiError(409, "LAST_PERMISSION_MANAGER", "마지막 활성 권한 관리자는 삭제할 수 없습니다.");
    }
  }
