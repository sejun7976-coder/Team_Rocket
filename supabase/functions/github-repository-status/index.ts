...
serve(async (request) => {
  ...
  const [{ data: member }, { data: project }] = await Promise.all([
    ...
    admin.from("projects").select("id, created_by, github_repository_id, github_owner, github_repository_name, github_repository_url, github_sync_status").eq("id", projectId).maybeSingle()
  ]);
  ...
  const upgradedRepository = marker === "legacy" || marker === "missing"
    ...
    : repository;
  const needsRecovery = marker !== "canonical"
    || project.github_repository_id !== upgradedRepository.id
    || project.github_owner?.toLowerCase() !== configuredOwner().toLowerCase()
    || project.github_repository_url !== upgradedRepository.html_url
    || project.github_sync_status !== "synced";
  if (needsRecovery) {
    const { data, error } = await admin.rpc("finalize_project_creation", { p_project_id: projectId, p_repository_id: upgradedRepository.id, p_owner: configuredOwner(), p_repository_name: upgradedRepository.name, p_repository_url: upgradedRepository.html_url });
