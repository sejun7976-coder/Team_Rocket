...
각 프로젝트는 독립 random 256-bit DEK를 사용한다. 사용자마다 P-256 ECDH key pair를 갖고, private key JWK는 로그인 password에서 PBKDF2-SHA-256으로 만든 KEK로 AES-256-GCM 암호화되어 `profiles`에 저장된다. password는 derivation 직후 폐기되고 keyring은 memory에만 존재한다.
...
새로고침 후 Supabase session은 복구되지만 private key는 복구되지 않으므로 사용자가 password로 keyring을 unlock해야 암호화 본문을 볼 수 있다. RLS 접근 권한과 복호화 가능성은 별개다.
...
- `src/stores`: auth/keyring/UI session memory. 민감값 persistence 금지.
